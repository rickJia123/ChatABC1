...
fun Intent.putReferrerTrackNode(activity: Activity): Intent =
...
fun View.createTrackBundle(): Bundle = Bundle().apply {
    ...
    putStringArray(
        KEY_TRACK_THREAD_NODES,
        findThreadNodeSet()?.map { it?.javaClass?.name }?.toTypedArray()
    )
}
...
fun Activity.postTrack(eventName: String, vararg classes: Class<*>) =
    window.decorView.postTrack(eventName, *classes)
...
fun ComponentActivity.putThreadTrackNode(trackNode: TrackNode) {
    val threadNodeSet =
        HashSet((window.decorView.getTag(R.id.tag_thread_nodes) as? Set<TrackNode?>).orEmpty())
